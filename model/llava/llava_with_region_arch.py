...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, **kwargs):
        image_features, image_forward_outs = self.get_model().get_vision_tower()(images, **kwargs)
        image_features = self.get_model().mm_projector(image_features)
        return image_features, image_forward_outs

    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, bboxes, **kwargs
    ):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features, image_forward_outs = self.encode_images(images, **kwargs)
        ...
        cur_image_idx = 0
        for batch_idx, (cur_input_ids, reg_feat) in enumerate(zip(input_ids, mlvl_reg_features)):
            curr_full_input_ids = []
            ...
            while image_token_indices.numel() > 0:
                ...
                elif getattr(self.config, "mm_use_im_start_end", False): # True
                    cur_input_ids = cur_input_ids[image_token_start + 2 :]
                else:
